...
window = GWindow(width=WINDOW_WIDTH, height=WINDOW_HEIGHT, title='trial')
score = 0
...
def main():
    ...
    while True:
        img = GImage('mole.png')
        rand_x = random.randint(0, window.width-img.width)
        rand_y = random.randint(0, window.height-img.height)
        window.add(img, x=rand_x, y=rand_y)
        pause(DELAY)
